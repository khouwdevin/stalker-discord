    ...
    for (let i = 0; i < guilds.size; i++) {
        ...
        if (channelId === "default") {
            if (!guild.systemChannel) {
                ...
                sendTimedMessageToExistingChannel(channels, "Please add or update default text channel to Stalker's config!", 10000)
...
export const notifyToConfigDefaultTextChannel = (channels: ChannelManager) => {
    for (let i = 0; i < channels.cache.size; i++){
        ...
        if (!channelGuild) continue
        if (!channelGuild.isTextBased() || channelGuild.isDMBased()) continue
        if (!channelGuild.guild.systemChannel) return sendTimedMessageToExistingChannel(channels, "Please add or update default text channel to Stalker's config!", 10000)
            
        const channel = channelGuild.guild.systemChannel
        return channel.send("Please add or update default text channel to Stalker's config!")
