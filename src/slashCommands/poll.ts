...
const command: SlashCommand = {
  ...
  execute: async (interaction) => {
    try {
      ...
      const currentMinutes =
        minutesNumber >= 10 ? minutesNumber : `0${minutesNumber}`
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle(
          `Poll's Subject: ${options[0].value?.toString().toUpperCase()}`
        )
        .setFooter({
          text: `poll is still ongoing until minutes ${currentMinutes}!`,
          iconURL: undefined,
        })
        .setColor('Blue')
      ...
      const timeout = setTimeout(async () => {
        ...
        if (!interaction.channel)
          return logger.error(
            '[Poll Slash Command]: ❌ Failed to execute poll slash command : channel unavailable'
          )
        ...
        try {
          const timeoutMessage = await interaction.channel.messages
            ...
            .catch((e) =>
              logger.error(
                `[Poll Slash Command]: ❌ Failed to fetch message : ${e.message}`
              )
            )
          ...
          const embed = new EmbedBuilder()
            ...
            .setFooter({ text: 'poll is over!', iconURL: undefined })
            .setColor('Blue')

          embed.addFields({ name: ' ', value: winnerResult })
      ...
      logger.error(
        `[Poll Slash Command]: ❌ Failed to execute poll : ${e.message}`
      )
    }
  },
  button: async (interaction) => {
    try {
      ...
      if (!interaction.channel)
        return await interaction.editReply('Channel has been deleted!')
      ...
      const targetMessage = await interaction.channel.messages
        ...
        .catch((e) =>
          logger.error(
            `[Poll Slash Command]: ❌ Failed to fetch message on button : ${e.message}`
          )
        )
      ...
      for (let i = 0; i < targetPoll.pollResult.length; i++) {
        if (i === option - 1) {
          if (targetPoll.usersID.includes(userID)) {
            targetPoll.pollResult[option - 1] =
              targetPoll.pollResult[option - 1] - 1
            ...
            for (let i = 0; i < targetPoll.pollResult.length; i++) {
              const userLength =
                targetPoll.usersID.length < 1 ? 1 : targetPoll.usersID.length
              const percentage = (targetPoll.pollResult[i] / userLength) * 100

              targetMessageEmbed.fields[
                i
              ].value = `${emojies[i]} ${targetPoll.pollResult[i]} votes (${percentage}%)`
            }
            ...
            const timeout = setTimeout(async () => {
              ...
              try {
                const timeoutMessage = await interaction.channel.messages
                  ...
                  .catch((e) =>
                    logger.error(
                      `[Poll Slash Command]: ❌ Failed to delete message on button : ${e.message}`
                    )
                  )
                ...
                const winner = result.every((val) => val === result[0])
                  ? -1
                  : Math.max(...result)
                const winnerResult =
                  winner === -1
                    ? '**THE RESULT IS TIE!**'
                    : `**[${emojies[winner]}] ${embedTimeout.fields[winner].name} IS THE WINNER!**`
          ...
          for (let i = 0; i < targetPoll.pollResult.length; i++) {
            const percentage =
              (targetPoll.pollResult[i] / targetPoll.usersID.length) * 100
      ...
      logger.error(
        `[Poll Slash Command]: ❌ Failed to vote in poll slash command on button : ${e.message}`
      )
    }
  },
  cooldown: 2,
