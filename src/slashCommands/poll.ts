  ...
  execute: async (interaction) => {
    try {
        ...
        async () => {
          ...
          if (!interaction.channel)
            return console.log(
              color(
                "text",
                `❌ Failed to execute poll slash command : ${color("error", "channel unavailable")}`,
              ),
            );
      ...
      console.log(
        color(
          "text",
          `❌ Failed to execute poll : ${color("error", e.message)}`,
        ),
      );
    }
  },
  button: async (interaction) => {
    try {
        ...
        if (i === option - 1) {
          if (targetPoll.usersID.includes(userID)) {
            ...
            for (let i = 0; i < targetPoll.pollResult.length; i++) {
              ...
              targetMessageEmbed.fields[i].value =
                `${emojies[i]} ${targetPoll.pollResult[i]} votes (${percentage}%)`;
            }
      ...
      console.log(
        color(
          "text",
          `❌ Failed to vote in poll slash command : ${color("error", e.message)}`,
        ),
      );
    }
  },
