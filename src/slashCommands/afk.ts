    ...
    try {
      ...
      if (!interaction.channel)
        return console.log(
          color(
            "text",
            `❌ Failed to execute AFK slash command : ${color("error", "channel unavailable")}`,
          ),
        );

      const options = interaction.options.data;
      const minutes: number = (options[1].value as number)
      ...
      if (isAFK) {
        ...
        await interaction.channel?.send(
          `${member} is AFK for ${userminutesafk}`,
        );
      } else {
        member
          .setNickname(member.user.displayName.replace("[AFK]", ""))
          .catch((e) => {});
      ...
      console.log(
        color(
          "text",
          `❌ Failed to execute AFK slash command : ${color("error", e.message)}`,
        ),
      );
    }
  },
